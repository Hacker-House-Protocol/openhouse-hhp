...
const SIZE_CFG = {
  xs: { icon: "w-[70%] aspect-square", emoji: "text-4xl", label: "text-[9px]", pad: "p-0.5 gap-0.5", remove: "size-3.5 text-[8px]" },
  sm: { icon: "w-[70%] aspect-square", emoji: "text-4xl", label: "text-[10px]", pad: "p-0.5 gap-0.5", remove: "size-4 text-[9px]" },
  md: { icon: "w-[70%] aspect-square", emoji: "text-5xl", label: "text-[11px]", pad: "p-1 gap-0.5", remove: "size-4 text-[10px]" },
}
...
export function SkillCard({ skill, selected, onRemove, onClick, size = "md" }: SkillCardProps) {
  ...
  return (
    <div
      ...
      className={cn(
        "relative flex flex-col items-center justify-center rounded-lg border text-center transition-all duration-200 aspect-square",
        s.pad,
    ...
    >
      ...
      {/* Label */}
      <p className={cn(
        "font-mono text-foreground leading-tight line-clamp-2 w-full",
        s.label,
      )}>
        {def.label}
      </p>
